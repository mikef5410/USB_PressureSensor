...
#define PSENSOR_CONVERSION_TIME_OSR_256			1000   // usec
...
#define MAX_CONVERSION_TIME				HSENSOR_CONVERSION_TIME_12b
  ...
  static enum ms8607_status
      hsensor_humidity_conversion_and_read_adc(uint16_t *);
  ...
  static enum ms8607_status psensor_conversion_and_read_adc(uint8_t,
							    uint32_t *);
  static bool psensor_crc_check(uint16_t * n_prom, uint8_t crc);
  enum ms8607_status psensor_read_pressure_and_temperature(float *,
							   float *);
  ...
  void ms8607_init(void) {
    ...
    psensor_resolution_osr = ms8607_pressure_resolution_osr_4096;
   ...
   */
  enum ms8607_status ms8607_set_humidity_resolution(enum
						    ms8607_humidity_resolution
						    res) {
    enum ms8607_status status;
   ...
   *       - ms8607_status_ok
   */
  void ms8607_set_humidity_i2c_master_mode(enum
					   ms8607_humidity_i2c_master_mode
					   mode) {
   ...
   *       - ms8607_status_crc_error : CRC check error
   */
  enum ms8607_status ms8607_read_temperature_pressure_humidity(float *t,
							       float *p,
							       float *h) {
    enum ms8607_status status;
  ...
  enum ms8607_status hsensor_humidity_conversion_and_read_adc(uint16_t *
    ...
    if (hsensor_i2c_master_mode == ms8607_i2c_hold) {
      status =
	  hsensor_write_command_no_stop
	  (HSENSOR_READ_HUMIDITY_W_HOLD_COMMAND);
    } else {
      status =
	  hsensor_write_command(HSENSOR_READ_HUMIDITY_WO_HOLD_COMMAND);
   ...
   */
  enum ms8607_status ms8607_get_compensated_humidity(float temperature,
						     float
						     relative_humidity,
						     float
   ...
   *       - ms8607_status_heater_on_error : Cannot compute compensated humidity because heater is on
   */
  enum ms8607_status ms8607_get_dew_point(float temperature,
    ...
    partial_pressure =
	pow(10,
	    HSENSOR_CONSTANT_A - HSENSOR_CONSTANT_B / (temperature +
						       HSENSOR_CONSTANT_C));

    *dew_point =
	-HSENSOR_CONSTANT_B /
	(log10(relative_humidity * partial_pressure / 100) -
	 HSENSOR_CONSTANT_A) - HSENSOR_CONSTANT_C;
  ...
  enum ms8607_status psensor_read_eeprom(void) {
    ...
    for (i = 0; i < COEFFICIENT_NUMBERS; i++) {
      status =
	  psensor_read_eeprom_coeff(PROM_ADDRESS_READ_ADDRESS_0 + i * 2,
				    eeprom_coeff + i);
      if (status != ms8607_status_ok)
	return status;
    }

    if (!psensor_crc_check
	(eeprom_coeff, (eeprom_coeff[CRC_INDEX] & 0xF000) >> 12))
      return ms8607_status_crc_error;
   ...
   *       - ms8607_status_no_i2c_acknowledge : I2C did not acknowledge
   */
  static enum ms8607_status psensor_conversion_and_read_adc(uint8_t cmd,
							    uint32_t *
							    adc) {
    enum ms8607_status status;
    enum status_code i2c_status;
    ...
    delay_ms(psensor_conversion_time
	     [(cmd & PSENSOR_CONVERSION_OSR_MASK) / 2] / 1000);
    if (status != ms8607_status_ok)
      return status;
							   ...
							   float *pressure)
  {
    ...
    // OFF = OFF_T1 + TCO * dT
    OFF =
	((int64_t) (eeprom_coeff[PRESSURE_OFFSET_INDEX]) << 17) +
	(((int64_t) (eeprom_coeff[TEMP_COEFF_OF_PRESSURE_OFFSET_INDEX]) *
	  dT) >> 6);
    OFF -= OFF2;
	...
	(((int64_t) eeprom_coeff[TEMP_COEFF_OF_PRESSURE_SENSITIVITY_INDEX]
	  * dT) >> 7);
    SENS -= SENS2;
  ...
  // I2C Wrappers for our board setup (STM32F3 I2C2)
  //
  void i2c_master_init(void) {
  ...
  enum status_code i2c_master_read_packet_wait(struct i2c_master_packet
					       *const packet) {
    i2c_transfer7(I2C2, packet->address, NULL, 0, packet->data,
		  packet->data_length);
    return (STATUS_OK);
  }
  ...
  enum status_code i2c_master_write_packet_wait(struct i2c_master_packet
						*const packet) {
    i2c_transfer7(I2C2, packet->address, packet->data, packet->data_length,
		  NULL, 0);
    return (STATUS_OK);
  }

  enum status_code i2c_master_write_packet_wait_no_stop(struct
							i2c_master_packet
							*const packet) {
    uint16_t addr = packet->address;
    uint16_t wn = packet->data_length;
    uint8_t *w = packet->data;

    i2c_set_7bit_address(I2C2, addr);
    i2c_set_write_transfer_dir(I2C2);
    i2c_set_bytes_to_transfer(I2C2, wn);
    i2c_disable_autoend(I2C2);
    i2c_send_start(I2C2);
	...
	while (i2c_nack(I2C2));	/* FIXME Some error */
      }
      i2c_send_data(I2C2, *w++);
    }
